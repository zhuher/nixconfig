...
o.shortmess = "a" -- short messaging such as changed([+]) etc.
...
o.termguicolors = true -- enable 24-bit color support
...
o.guicursor =
[[n-v-c:block,i-ci-ve:ver25,r-cr:hor20,o:hor50,a:blinkwait700-blinkoff400-blinkon250-Cursor/lCursor,sm:block-blinkwait175-blinkoff150-blinkon175]] -- gui cursor settings
o.foldmethod =
"marker"                                                                                                                                           -- fold using markers
...
for type, symbol in pairs(symbols) do
  opts.signs.text[vim.diagnostic.severity[type]] = symbol
end
...
-- vim.lsp {{{
vim.lsp.config('*', { -- {{{
  root_markers = { '.git', '.jj', '.direnv', '.envrc' },
  capabilities = {
    textDocument = {
      semanticTokens = {
...
local plugins = {
  ...
  -- fzf-lua {{{
  {
    ...
    dependencies = { { "echasnovski/mini.icons", lazy = true, module = true },
      -- { "roosta/fzf-folds.vim", lazy = true, cmd = "Folds", module = true },
    },
    opts = {},
    cmd = "FzfLua",
    keys = {
